...
async function testSchemaSync() {
  ...
  try {
    ...
    const backendResponse = await fetch(`${BASE_URL}/api/seo/pages?page_path=/blog/[slug]`);
    ...
    console.log('✅ 后端SEO API响应:', {
      ...
      has_blog_slug_config: backendData.pages?.some(p => p.page_path === '/blog/[slug]') || false
    });

    // 2. 测试前端SEO API代理
    console.log('\n2️⃣ 测试前端SEO API代理...');
    const frontendResponse = await fetch(`${FRONTEND_URL}/api/seo/pages?page_path=/blog/[slug]`);
    ...
    console.log('✅ 前端SEO API代理响应:', {
      ...
      has_blog_slug_config: frontendData.pages?.some(p => p.page_path === '/blog/[slug]') || false
    });

    // 3. 检查具体的schema配置
    const blogSlugConfig = frontendData.pages?.find(p => p.page_path === '/blog/[slug]');
    if (blogSlugConfig) {
      console.log('\n3️⃣ 博客页面SEO配置详情:');
      ...
      console.log('✅ Schema配置:', blogSlugConfig.schema_markup ? '已配置' : '未配置');
    ...
    if (blogSlugConfig?.schema_markup) {
      ...
      console.log('📝 模板预览:', blogSlugConfig.schema_markup.substring(0, 200) + '...');
    } else {
