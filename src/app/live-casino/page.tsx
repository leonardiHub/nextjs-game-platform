...
import LiveCasinoPageClient from './LiveCasinoPageClient'
import { getPageSEOSettings, getGlobalSEOSettings } from '@/utils/seo'

export async function generateMetadata(): Promise<Metadata> {
  try {
    // 尝试获取页面特定的SEO设置
    const pageSEO = await getPageSEOSettings('/live-casino')
    const globalSettings = await getGlobalSEOSettings()

    console.log('Live Casino page SEO data:', pageSEO)
    ...
    if (finalPageSEO) {
      ...
      const canonicalUrl =
        ...
        (globalSettings.default_canonical_url
          ? `${globalSettings.default_canonical_url}/live-casino`
          : undefined)
    ...
    console.error('Error generating live casino page metadata:', error)
    return {
      title: 'Live Casino - 99Group Gaming Platform',
      description:
        'Experience the thrill of live casino games with real dealers at 99Group. Play live blackjack, roulette, baccarat and more.',
      robots: 'index, follow',
    }
  }
}
