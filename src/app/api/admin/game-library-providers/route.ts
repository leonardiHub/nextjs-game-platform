...
db.get('SELECT COUNT(*) as count FROM game_library_providers', (err, row: any) => {
  if (!err && row.count === 0) {
      ...
      if (!err && brands && brands.length > 0) {
        ...
        brands.forEach(brand => {
          db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
          `, [brand.id, brand.name, brand.code, brand.logo_url, brand.description, brand.status, brand.games_count, brand.created_at, brand.updated_at])
        })
      } else {
        ...
        defaultProviders.forEach(provider => {
          db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `, [provider.name, provider.code, provider.logo_url, provider.description, provider.status, provider.games_count])
        })
      }
    })
  }
