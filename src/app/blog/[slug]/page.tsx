...
import { processSchemaTemplate, getPageSchemaTemplate } from '@/utils/schemaProcessor'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = blog.seo_description || blog.excerpt || 'Read our latest blog post'
  ...
  try {
    ...
    if (schemaTemplate) {
      ...
      console.log('✨ Processed schema:', processedSchema ? 'Success' : 'Failed')
      ...
      if (processedSchema) {
        try {
          ...
          console.error('❌ Failed to parse processed schema as JSON:', parseError)
        }
      }
    } else {
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (blog) {
    try {
      ...
      const schemaTemplate = await getPageSchemaTemplate('/blog/[slug]')
      console.log('📋 Page component: Schema template received:', schemaTemplate ? 'Yes' : 'No')
      
      if (schemaTemplate) {
        console.log('⚙️  Page component: Processing schema template')
        const processedSchema = processSchemaTemplate(schemaTemplate, blog)
        console.log('🎯 Page component: Processed schema result:', processedSchema ? 'Success' : 'Failed')
        
        if (processedSchema) {
          try {
            structuredData = JSON.parse(processedSchema)
            console.log('✅ Page component: Schema JSON parsed successfully')
            console.log('📊 Page component: Final structured data:', JSON.stringify(structuredData, null, 2))
          } catch (parseError) {
            console.error('❌ Page component: Failed to parse schema JSON:', parseError)
          }
        }
      } else {
      ...
      console.error('❌ Page component: Error processing schema template:', error)
    }
  } else {
    console.warn('⚠️  Page component: No blog data available for schema processing')
  }
