...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const loadStats = async () => {
    try {
      ...
      setStats({
        ...
        pendingWithdrawals:
          withdrawalsData.withdrawals?.filter(
            (w: any) => w.status === 'pending'
          ).length || 0,
        pendingKYC:
          kycData.kyc_documents?.filter((k: any) => k.status === 'submitted')
            .length || 0,
  ...
  const getPageTitle = () => {
    switch (activeTab) {
      ...
      case 'game-settings':
        return 'Platform Provider Settings'
      ...
      case 'seo-settings':
        return 'SEO Settings'
      case 'advertising-settings':
        return 'Advertising Settings'
  ...
  const getPageSubtitle = () => {
    switch (activeTab) {
      ...
      case 'kyc':
        return 'Review and approve identity verification documents'
      case 'user-analytics':
        return 'Analyze user behavior and engagement'
      case 'withdrawals':
        return 'Process withdrawal requests and reviews'
      case 'transactions':
        return 'View all transaction records'
      case 'payment-settings':
        return 'Configure payment methods and settings'
      case 'game-library':
        return 'Manage available games and content'
      case 'game-providers':
        return 'Manage game providers (JILI, Pragmatic Play, etc.)'
      case 'game-analytics':
        return 'Analyze game performance and user engagement'
      case 'game-settings':
        return 'Configure merchant platform connection and API settings'
      case 'business-reports':
        return 'Business performance and analytics'
      case 'financial-reports':
        return 'Financial statements and revenue analysis'
      case 'user-behavior':
        return 'User activity patterns and insights'
      case 'security-logs':
        return 'Security events and access logs'
      case 'system-settings':
        return 'System parameters and configuration'
      case 'admin-accounts':
        return 'Manage administrator accounts and roles'
      case 'blog-management':
        return 'Manage blog posts and content'
      case 'blog-editor':
        return 'Create and edit blog posts'
      case 'media-library':
        return 'Manage images and media files'
      case 'categories-tags':
        return 'Organize content with categories and tags'
      case 'seo-settings':
        return 'Search engine optimization settings'
      case 'advertising-settings':
        return 'Configure tracking pixels, conversion APIs, and advertising integrations'
      case 'campaign-tracking':
        return 'Monitor advertising campaign performance and metrics'
      case 'conversion-reports':
        return 'Analyze conversion data and advertising ROI'
      case 'notifications':
        return 'System notifications and message management'
      case 'email-templates':
        return 'Manage email templates and communications'
      case 'announcements':
        return 'System announcements and updates'
      default:
        return ''
    }
  }
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <AdminHeader title={getPageTitle()} subtitle={getPageSubtitle()} />
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {activeTab === 'user-analytics' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                User analytics feature is under development...
              </p>
            </div>
          )}
          ...
          {activeTab === 'payment-settings' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Payment settings feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'game-library' && <GameLibraryManagement />}
          {activeTab === 'game-providers' && <GameProviderManagement />}
          {activeTab === 'game-analytics' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Game analytics feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'game-settings' && <PlatformProviderSettings />}
          {activeTab === 'business-reports' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Business reports feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'financial-reports' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Financial reports feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'user-behavior' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                User behavior analysis feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'security-logs' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Security logs feature is under development...
              </p>
            </div>
          )}
          {activeTab === 'admin-accounts' && <AdminAccountManagement />}
          {activeTab === 'blog-management' && <BlogManagement />}
          {activeTab === 'blog-editor' && (
            <BlogEditor blogId={editingBlogId} onBack={handleBackToBlogList} />
          )}
          {activeTab === 'media-library' && <MediaLibrary />}
          {activeTab === 'categories-tags' && <CategoriesAndTags />}
          {activeTab === 'hero-carousel' && <HeroCarouselManagement />}
          {activeTab === 'seo-settings' && <SEOSettings />}
          {activeTab === 'advertising-settings' && <AdvertisingSettings />}
          {activeTab === 'campaign-tracking' && <CampaignTracking />}
          {activeTab === 'conversion-reports' && <ConversionReports />}
          {activeTab === 'email-templates' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-gray-600">
                Email templates feature is under development...
              </p>
            </div>
          )}
